...
_COL_MAP = {
    ...
    "nome do preposto":     "nome_preposto",
    "e-mail do preposto":   "email_preposto",
    "email do preposto":    "email_preposto",
    "telefone do preposto": "telefone_preposto",
    "cpf do preposto":      "cpf_preposto",
    # snake_case aliases (retrocompatibilidade)
    "numero_processo":      "numero_processo",
    "nome_preposto":        "nome_preposto",
    "email_preposto":       "email_preposto",
    "telefone_preposto":    "telefone_preposto",
    "cpf_preposto":         "cpf_preposto",
}
...
def upload():
    ...
    preview = [
        {"processo": r["numero_processo"], "nome": r["nome_preposto"],
         "telefone": str(r.get("telefone_preposto") or "")}
        for r in rows[:5]
    ]
...
def download():
    ...
    path = OUTPUT_DIR / sid / "relatorio_prepostos.xlsx"
    if not path.exists():
...
def _parse_spreadsheet(path: Path) -> list[dict]:
    ...
    required = {"numero_processo", "nome_preposto", "cpf_preposto"}
    missing  = required - present
    if missing:
        raise ValueError(
            f"Coluna(s) ausente(s): {', '.join(sorted(missing))}. "
            "Esperado: 'Número do Processo', 'Nome do Preposto', 'CPF do Preposto'."
        )
    ...
    for row in rows_iter:
        ...
        if not any(data.values()):
            ...
            continue
        vazias = 0
        if not data.get("numero_processo") or not str(data.get("nome_preposto") or "").strip():
            continue
        records.append(data)

    wb.close()
    if not records:
        raise ValueError("Nenhum dado válido encontrado (linhas sem Nome do Preposto são ignoradas)")
    return records
...
if __name__ == "__main__":
    print("Preposto RPA → http://localhost:5001")
